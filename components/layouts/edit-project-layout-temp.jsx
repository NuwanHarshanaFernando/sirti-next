...
const EditProjectLayout = () => {
  ...
  const handleAddNewRacks = async () => {
    ...
    if (emptyRacks.length === 0) {
      alert("Please add some rack slots first by clicking 'Add Rack'");
      return;
    }
    ...
    try {
      ...
      alert(`${result.racks.length} new racks created successfully!`);
      ...
      alert(`Failed to add racks: ${error.message}`);
    }
  };
