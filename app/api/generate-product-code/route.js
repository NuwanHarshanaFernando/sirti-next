...
export async function GET() {
  return NextResponse.json({ 
    message: "Generate product code API is working",
...
export async function POST() {
  try {
    ...
    if (attempts >= maxAttempts * 2) {
      console.error("Failed to generate unique product code after maximum attempts");
      return NextResponse.json(
        { error: "Unable to generate unique product code after multiple attempts" },
        { status: 500 }
      );
    }
